...
class AviaNZ_batchProcess:
    ...
    def detect(self):
        ...
        for root, dirs, files in os.walk(str(self.dirName)):
            for filename in files:
                if (
                    (self.method != "Click" and filename.lower().endswith(".wav"))
                    or (self.method == "Click" and filename.lower().endswith(".bmp"))
                    or (self.method == "Bats" and filename.lower().endswith(".bmp"))
                ):
        ...
        if self.CLI or self.testmode:
            ...
            timeWindow_s = (
                self.ui.w_timeStart.time().hour() * 3600
                + self.ui.w_timeStart.time().minute() * 60
                + self.ui.w_timeStart.time().second()
            )
            timeWindow_e = (
                self.ui.w_timeEnd.time().hour() * 3600
                + self.ui.w_timeEnd.time().minute() * 60
                + self.ui.w_timeEnd.time().second()
            )
        ...
        if not self.testmode:
            ...
            if self.log.possibleAppend:
                ...
                text = (
                    "Previous analysis found in this folder (analysed "
                    + str(len(filesExistAndDone))
                    + " out of "
                    + str(total)
                ...
                if not self.CLI:
                    ...
                    if (
                        confirmedResume.lower() == "yes"
                        or confirmedResume.lower() == "y"
                    ):
                        # ignore files in log
                        self.filesDone = filesExistAndDone
                    else:
            ...
            text = (
                "Analysis will be launched with these settings:\n" + text + "\nConfirm?"
            )
        ...
        if not self.testmode:
            ...
            if self.method == "Click" or self.method == "Bats":
                if not self.CLI:
                    ...
                    self.mutex.lock()
                    self.need_bat_info.emit(
                        self.config["operator"], easting, northing, recorder
                    )
                    self.ui.msgClosed.wait(self.mutex)
                    self.mutex.unlock()
    ...
    def mainloop(self, allwavs, total, speciesStr, filters, settings):
        ...
        for filename in allwavs:
            ...
            with open(filename, "br") as f:
                if (
                    (self.method == "Click" and f.read(2) != b"BM")
                    or (self.method == "Bats" and f.read(2) != b"BM")
                    or (
                        self.method != "Click"
                        and self.method != "Bats"
                        and f.read(4) != b"RIFF"
                    )
                ):
                    print(
                        "Warning: file %s not formatted correctly, skipping" % filename
                    )
                    self.log.appendFile(filename)
                    continue
                ...
                for speciesix in range(len(filters)):
                    ...
                    if self.method == "Click":
                        ...
                        if (
                            ...
                            thisPageSegs = self.ws.waveletSegment(
                                speciesix, wpmode="new"
                            )
                            ...
                            thisPageSegs = self.ws.waveletSegmentChp(
                                speciesix, alg=2, wind=self.wind
                            )
                        else:
                            print(
                                "ERROR: unrecognized method",
                                filters[speciesix]["method"],
                            )
                            raise Exception
                            ...
                            if self.method == "Click":
                                ...
                                if click_label == "Click":
                                    ...
                                    label = self.File_label(
                                        predictions, thr1=thr1, thr2=thr2
                                    )
                                ...
                                self.method == "Bats"
                            ):  # Let's do it here - PostProc class is not supporting bats
                                ...
                                specFrameSize = len(
                                    range(
                                        0,
                                        int(
                                            CNNmodel[1][0] * self.sp.sampleRate
                                            - self.sp.window_width
                                        ),
                                        self.sp.incr,
                                    )
                                )
                                for i in range(int(n)):
                                    ...
                                    featuress.append(
                                        [np.rot90(sgRaw_i / maxg).tolist()]
                                    )
                                featuress = np.array(featuress)
                                featuress = featuress.reshape(
                                    featuress.shape[0],
                                    CNNmodel[2][0],
                                    CNNmodel[2][1],
                                    1,
                                ...
                                if isinstance(probs, int):
                                    ...
                                    if n > 4:
                                        n = 4
                                    prob = [
                                        np.mean(probs[ind[0][-n // 2 :], 0]),
                                        np.mean(probs[ind[1][-n // 2 :], 1]),
                                        (
                                            np.sum(probs[ind[0][-n // 2 :], 2])
                                            + np.sum(probs[ind[1][-n // 2 :], 2])
                                        )
                                        / (n // 2 * 2),
                                    ]
    ...
    def ClickSearch(self, imspec, file, virginia=True):
        ...
        thr_spec = (np.mean(mean_spec_all) + np.std(mean_spec_all)) * np.ones(
            (np.shape(mean_spec))
        )
            ...
            for i in range(1, np.shape(clicks_indices)[1]):
                if clicks_indices[0][i] == click_end + 1:
                    ...
                    if click_end - click_start + 1 > up_len:
                        ...
                        # savedataset
                        featuress, count = self.updateDataset(
                            file, featuress, count, imspec, click_start, click_end, dt
                        )
                    # update
                ...
                for count in range(nfiles):
                    ...
                    if filename.endswith(".data"):
                        ...
                        if len(segments) > 0:
                            ...
                            self.sp.readBmp(
                                os.path.join(root, fn), rotate=False, silent=True
                            )
                            self.sampleRate = self.sp.sampleRate
    ...
    def exportToBatSearch(
        self, dirName, savefile="BatData.xml", threshold1=0.85, threshold2=0.7
    ):
    ...
    def exportBatSurvey(self, dirName, responses, threshold1=0.85):
        ...
        f.write(
            "Data Source,Observer,Survey method,Species,Passes,Date,Detector type,Date recorder put out,Date recorder collected,No. of nights out,Effective nights out,Notes,Eastings,Northings,Site name,Region\n"
        )
    ...
    def exportToBatSearchCSV(
        self, dirName, writefile="BatResults.csv", threshold1=0.85, threshold2=0.7
    ):
