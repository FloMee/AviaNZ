...
class CNNtrain:
    def __init__(
        ...
        self.sp = SignalProc.SignalProc(
            self.LearningDict["sgramWindowWidth"], self.LearningDict["sgramHop"]
        )
                    ...
                    if file.lower().endswith(".wav") and file + ".corrections" in files:
                        ...
                        for seg in annots:
                            ...
                            if len(oldlabel) != 1:
                                ...
                                continue
                            if (
                                oldlabel[0]["species"] == self.species
                                and self.species not in newsp
                            ):
                                # store this as "noise" calltype
                                self.traindata.append(
                                    [wavfile, seg[0][:2], len(self.calltypes)]
                                )
                                self.correction = True
                    elif (
                        file.lower().endswith(".wav")
                        and file + ".corrections_" + self.cleanSpecies(self.species)
                        in files
                    ):
                        # Read the .correction (from single sp review)
                        cfile = os.path.join(
                            root,
                            file + ".corrections_" + self.cleanSpecies(self.species),
                        )
                        wavfile = os.path.join(root, file)
                        try:
                            f = open(cfile, "r")
                        ...
                        for seg in annots:
                            if isinstance(seg, dict):
                                ...
                                # store this as "noise" calltype
                                self.traindata.append(
                                    [wavfile, seg[:2], len(self.calltypes)]
                                )
                                self.correction = True
    ...
    def genImgDataset(self, hop):
        ...
        self.imgsize[1], self.Nimg = self.DataGen.generateFeatures(
            dirName=self.tmpdir1.name, dataset=self.traindata, hop=hop
        )
    ...
    def train(self):
        ...
        print("Training...")
        cnn.train(
            modelsavepath=self.tmpdir2.name,
        ...
        for i in range(int(np.ceil(N / self.LearningDict["batchsize_ROC"]))):
            ...
            imagesb = cnn.loadImgBatch(
                X_val_filenames[
                    i
                    * self.LearningDict["batchsize_ROC"] : min(
        ...
        self.TPRs = [
            [0.0 for i in range(len(self.Thrs))] for j in range(len(self.calltypes) + 1)
        ]
        self.FPRs = [
            [0.0 for i in range(len(self.Thrs))] for j in range(len(self.calltypes) + 1)
        ]
        ...
        fig, axs = plt.subplots(
            len(self.calltypes) + 1, len(self.calltypes) + 1, sharey=True, sharex="col"
        )

        for ct in range(len(self.calltypes) + 1):
            self.TPRs[ct] = [
                TPs[ct][i] / (TPs[ct][i] + FNs[ct][i]) for i in range(len(self.Thrs))
            ]
            self.FPRs[ct] = [
        ...
        if self.autoThr:
            for ct in range(len(self.calltypes)):
                ...
                distarr = (np.float64(1) - self.TPRs[ct]) ** 2 + (
                    np.float64(0) - self.FPRs[ct]
                ) ** 2
                self.thr_min_ind = np.unravel_index(np.argmin(distarr), distarr.shape)[
                    0
                ]
    ...
    def updateHop(self, imgN, hop):
        ...
        for i in range(len(self.calltypes) + 1):
            fillratio1 = imgN[i] / (
                self.LearningDict["t"] - self.LearningDict["tWidthShift"]
            )
            fillratio2 = imgN[i] / self.LearningDict["t"]
            if fillratio1 < 0.75:  # too less, decrease hop
                if i == len(self.calltypes):
                    print(
                        "Noise: only %d images, adjusting hop from %.2f to %.2f"
                        % (imgN[i], hop[i], hop[i] * fillratio1)
                    )
                else:
                    print(
                        "%s: only %d images, adjusting hop from %.2f to %.2f"
                        % (self.calltypes[i], imgN[i], hop[i], hop[i] * fillratio1)
                    )
                hop[i] = hop[i] * fillratio1
            elif (
                fillratio1 > 1 and fillratio2 > 0.75
            ):  # increase hop and make room for augmenting
                if i == len(self.calltypes):
                    print(
                        "Noise: %d images, adjusting hop from %.2f to %.2f"
                        % (imgN[i], hop[i], hop[i] * fillratio1)
                    )
                else:
                    print(
                        "%s: %d images, adjusting hop from %.2f to %.2f"
                        % (self.calltypes[i], imgN[i], hop[i], hop[i] * fillratio1)
                    )
                hop[i] = hop[i] * fillratio1
                ...
                if i == len(self.calltypes):
                    print(
                        "Noise: %d images, adjusting hop from %.2f to %.2f"
                        % (imgN[i], hop[i], hop[i] * fillratio2)
                    )
                else:
                    print(
                        "%s: %d images, adjusting hop from %.2f to %.2f"
                        % (self.calltypes[i], imgN[i], hop[i], hop[i] * fillratio2)
                    )
        ...
        for root, dirs, files in os.walk(self.testDir):
            for file in files:
                ...
                if (
                    ...
                    GT = self.loadGT(
                        os.path.join(root, file[:-4] + "-GT.txt"), duration
                    )
