...
class SplitData(QMainWindow):
    ...
    def browseO(self):
        if self.dirO:
            self.dirO = QFileDialog.getExistingDirectory(
                self, "Choose Folder to Process", str(self.dirO)
            )
        else:
            self.dirO = QFileDialog.getExistingDirectory(
                self, "Choose Folder to Process"
        ...
        samedir = (
            self.dirO != ""
            and self.dirName != ""
            and os.path.samefile(self.dirO, self.dirName)
        )
        ...
        if not os.access(self.dirO, os.W_OK | os.X_OK):
            ...
            self.labelOut.setText(
                "ERROR: cannot use the same folder for input and output"
            )
            self.outdirOk = False
    ...
    def fillFileList(self):
        ...
        if len(self.listOfDataFiles) == 0:
            ...
            self.labelDatas.setText(
                "Found <b>%d</b> DATA files: %s..."
                % (len(self.listOfDataFiles), ", ".join(self.listOfDataFiles[:3]))
            )
            noData = False
    ...
    def split(self):
        ...
        for f in self.listOfWavs:
            ...
            try:
                ...
                outfile_c = (
                    outfile_c
                    + "_"
                    + dt.datetime.strftime(wavdt, "%Y%m%d_%H%M%S")
                    + ".wav"
                )
            ...
            if os.path.isfile(infile_c) and os.stat(infile_c).st_size > 100:
                ...
                succ = SplitLauncher.launchCython(
                    infile_c, outfile_c, self.cutLen, wavHasDt
                )
                if succ != 0:
                ...
                print(
                    "Warning: input file %s does not exist or is empty, skipping",
                    infile_c,
                )
        ...
        if donefiles == totalfiles:
            msg = SupportClasses_GUI.MessagePopup(
                "d", "Finished", "Folder processed successfully!"
            )
            msg.exec_()
        ...
        for a in range(len(all)):
            if time != 0:
                f2 = (
                    str(outprefix)
                    + "_"
                    + dt.datetime.strftime(time, "%Y%m%d_%H%M%S")
                    + ".wav.data"
                )
